...
- Names that don't exist when the module is imported are no longer added to `__all__`:
  names deleted with `del`, defined only under `if TYPE_CHECKING:`,
  `if __name__ == "__main__":` or `if False:`, comprehension variables, walrus targets
  in lambdas and annotations without a value (`X: int`)
